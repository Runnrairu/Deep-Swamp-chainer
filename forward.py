...
class param_gen(chainer.Chain):
    ...
    def __call__(self,t):
        if self.hypernet:
            ...
            W1=self.flowcon1_param.W*h_1.reshape(channel,1,1,1)
            W2=self.flowcon2_param.W*h_2.reshape(channel,1,1,1)
...
class flow_net(chainer.Chain):
    def __init__(self,task_name,hypernet,T,N,channel):
...
class model(chainer.Chain):
    def __init__(self, n_class,dense,channel,T,N,task_name,hypernet=False,first_conv=False):
        ...
        self.dense=dense
        self.train=True
        self.flow=flow_net(task_name,hypernet,T,N,channel)
